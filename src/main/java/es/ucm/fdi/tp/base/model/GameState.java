 ...
 * know which actions are valid and which ones are not, and to draw a
 * representation of the state. States are immutable - applying an action to a
 * state should must in a new GameState. - once created, states should be
 * unmodifiable
 */
public abstract class GameState<S extends GameState<S, A>, A extends GameAction<S, A>> implements Serializable {
	...
	public abstract void dBomba(int i);
	
	public abstract void bomba();
	/**
	...
	public abstract boolean outOfTheBoard(int row, int col);
	/**
	...
	public void save(File file) throws IOException {
		...
		try (ObjectOutputStream oos = new ObjectOutputStream(fout)) {
			...
			throw new IOException("Error saving to '" + file.getAbsolutePath() + "':", ioe);
		}
	}
